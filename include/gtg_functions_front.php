<?php
	...
	## 20 ##
	function GTG_add_to_cart($id,$q,$c,$s,$cflag,$wprice,$Mode,$lll,$aaa,$ggg,$gmmm)
	{
		...
		if(isset($_SESSION['P']))
		{
			...
			$GCARD = $_SESSION['GCARD'];
			$GCARDMSG = $_SESSION['GCARDMSG'];
			$flag = 0;
			...
			if($flag == 0)
			{
				...
				$GCARD[count($_SESSION['GCARD'])] = $ggg;
				$GCARDMSG[count($_SESSION['GCARDMSG'])] = $gmmm;
			}
			$_SESSION['P'] = $P;
			$_SESSION['Q'] = $Q;
			...
			$_SESSION['GCARDMSG'] = $GCARDMSG;
			$_SESSION['GCARD'] = $GCARD;
		}
		else
		{
			$P[0] = $id;
	...
	## 21 ##
	function GTG_remove_from_cart($id,$rem_color,$rem_size,$rem_LETER,$rem_ASIGN)
	{
		if(isset($_SESSION['P']))
		{
			$P = $_SESSION['P'];
			...
			if(1==2)
			{
				...
				session_unregister('GCARD');
				session_unregister('GCARDMSG');
			}
			else
			{
				for($i=0;$i<count($P);$i++)
				{
					if((trim($P[$i]) != trim($id)) || (trim($COLOR[$i])!=trim($rem_color)) || (trim($SIZE[$i])!=trim($rem_size)) || (trim($LETER[$i])!=trim($rem_LETER)) || (trim($ASIGN[$i])!=trim($rem_ASIGN)) )
					{
						$P_temp[$count] = $P[$i];
						$Q_temp[$count] = $Q[$i];
						...
						$GCARD_temp[$count] = $GCARD[$i];
						$GCARDMSG_temp[$count] = $GCARDMSG[$i];
				...
				$_SESSION['GCARD'] = $GCARD_temp;
				$_SESSION['GCARDMSG'] = $GCARDMSG_temp;
			}
